...
if (!empty($_GET)) {
	...
	if (!empty($_GET['dates'])) {
		...
		if ($validDateFrom && $validDateTo) {
			...
			$days = ($date_to - $date_from) / 86400;
			if ($days>1) $dateFormat = '%y.%m.W%v';
		}
	}
...
{
	$aGroup = array();
	$csvFile = file('https://docs.google.com/spreadsheets/d/e/2PACX-1vR2gY22xgcaR4JUr3naK5nXbFzw3pL_Ogn4msFRDGfVA8nILfEs-BOdxDRt2Jvhx9Yz31eAF8IfpjBn/pub?gid=0&single=true&output=tsv'); //change to the new TSV --John Chen 2019/11/12
	//$csvFile = file('./melon-dash-test.csv');
	foreach ($csvFile as $line) {
		$csv_check_data = str_getcsv($line,"\t"); //Add "\t" --John Chen 2019/11/12
		if (isset($_GET['tier'])) {
	...
	foreach ($data as $line) {
		$csv_product_asin = substr($line[1], strrpos($line[], '/') + 1); //Change 0 to 1 as now the 1st column is SR not URL --John Chen 2019/11/12
		...
		$line = $line[5]; //Change 4 to 5 --John Chen 2019/11/12
		if (empty($line)) {
			continue;
		}
		if (strpos($line, '--') > -1) {
...
function DrawChartLinearBar($points, $aMapData, $aTooltipData, $h = '', $title_x = '', $yLegents = '', $width = 1600, $height = 120)
{
	$plotType = 'linepoints';
...
{
	global $interval, $models, $assignee, $region, $sellers, $dnsCountry, $timeoffset, $dateFormat, $table, $date_sql, $aColors, $image_width, $aTooltipData, $aSkus, $sAsinsWithRank1NotToBeDevidedBy1000;
	...
	foreach ($aH as $h) {
		...
		$decimal = 0;
		$sh = substr($h, 0, 1) . substr($h, -2);
		if ($h == 'avgrating' || $h == 'reviews') $decimal = 1;
		if ($h == 'price') {
			$decimal = 2;
		}
		...
		if (is_array($aMapData) and !empty($aMapData)) DrawChartLinearBar($points, $aMapData, $aTooltipData, $h, 'time', '', $image_width, $image_height);
...
function MwsGroupNumbers($group, $aH, $alter = '')
{
	...
	foreach ($aH as $h) {
		...
		$qBaseSub .= ", ROUND(avg($h),$decimal) as $sh ";
		//TODO:need to caculate the points as much as possible on the screen resolution
		$points = (24 / $interval) + 2;
		$q1 = "SELECT dtime $q1Sub FROM (SELECT date_format(from_unixtime(time+$timeoffset),'$dateFormat') as dtime, asin $qBaseSub FROM $table WHERE asin IN ('$sAsins') AND $h > 0 AND $date_sql GROUP BY asin, dtime HAVING $sh>0 ) a GROUP BY dtime ORDER BY dtime DESC"; //limit 2
