...
<title>asin_uk_numbers_import</title>
<?php
set_time_limit(290);
...
define('LOGFILE','asin_uk_numbers_import.log.html');
if (filesize(LOGFILE) > 1234567) rename(LOGFILE,'old.'. LOGFILE);
$errorMsg = '';
$test = isset($_GET['test']) ? $_GET['test'] : 0;
$aFiles = glob("20*.asin_uk_numbers.csv");
if (empty($aFiles)) {
	$errorMsg = 'No 20*.asin_uk_numbers.csv in this hour';
} else {
	foreach($aFiles as $file) {
		...
		if ($test <> 1 && $result <> 'sql error') rename($file,'asin_uk_numbers_last.csv');
	}
}
if (strlen($errorMsg) > 10) sendmail('asin_uk_numbers_import error',$errorMsg);
...
function ImportNumbers($file) {
	...
	$asin_uk_numbers = file_get_contents($file);
	//~ echo $asin_uk_numbers .'<br>';
	$lines = explode("\n",$asin_uk_numbers);
	//2016-06-01 19:01:06	B003XELTTG	190.11	1773	4.8	282	1	RCC7
	$q1h = 'INSERT IGNORE INTO asin_uk_numbers (time, asin, price, lowest, soldby, reviews, avgrating, fiveneeded, p5, p4, p3, p2, p1, qa, rank1, rank2) VALUES '; //. trim($values1,',');
	$q2h = 'INSERT IGNORE INTO asin_uk_strings (asin,brand,sku,title,featuresPoints) VALUES '; // . trim($values2,',');
	$q3h = 'INSERT IGNORE INTO asin_uk_cps (asin,cps) VALUES '; // . trim($values3,',');
	$q4h = 'INSERT IGNORE INTO asin_uk_lrs (asin,LatestReviews) VALUES '; // . trim($values4,',');
	...
	if ($r <> false) {
		...
		echo $rows ." rows imported into asin_uk_numbers from $file<br>\n". str_replace(')',")<br>",$q_1234);
		file_put_contents(LOGFILE,date('Y-m-d H:i'). ' - '. $rows ." rows imported into asin_uk_numbers from $file\n", FILE_APPEND );
		return $rows;
	} else {
