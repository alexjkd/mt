...
function unzip($zipfile,$path_extracted_to='.') {
	if (!file_exists($zipfile)) {echo "Zip file $zipfile not found"; return -1;}
...
function asin_sku_conversion($s) {
	$asin = $sku = ''; $today = date('Ymd'); $tb = 'amazon_items'; //ispring_asin_sku
...
function drawchart($array2d,$title_c='',$title_x='',$yLegents='',$width=1600,$height=120){
	...
	if (count($array2d) >= 21) {$plot->SetXLabelAngle(45);}
...
function sendmail($subject,$message,$from='MarketTracker <john@ispringfilter.com>'){
	...
	if(strpos($send_mail_log,$subject)) {echo "<BR> $subject was already sent in email."; return -1;}
