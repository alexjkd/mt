...
function MwsGroupNumbers($group,$aH,$alter=''){
	...
	foreach($aH as $h) {
		...
		if ($h=='rank1') $q1="SELECT dtime $q1Sub FROM (SELECT date_format(DATE_SUB(updated, INTERVAL 4 HOUR),'$dateFormat') as dtime, asin $qBaseSub FROM mws.melon_mws_us WHERE asin IN ('$sAsins') AND $h > 0 AND updated >= FROM_UNIXTIME($days)  GROUP BY asin, dtime HAVING $sh>0 ) a GROUP BY dtime ORDER BY dtime DESC"; //limit 2
