	...
	foreach(explode('|',TooltipHeaders) as $h) {
		...
		for($ln=0;$ln<count($aLines);$ln++) {
			...
			if (stripos($legend,$asin)==false) {
				$legend .= "<span class=\"dot\" style=\"background-color:". $aColors[$ln] ."\"></span><a title=\"Open ". $asin ." at Amazon.com\" target=_blank href=\"http://www.amazon.". $dnsCountry ."/dp/$asin\">$sku</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
				$items .= $line .';';
				$aSkus[] = $sku;
				$aAsins[] = $asin;
			}
			...
			$sSkus = implode(',',$aSkus);
			if (preg_match($sAsinsWithRank1NotToBeDevidedBy1000,$group)==FALSE) {
